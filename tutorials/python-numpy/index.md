...
But what it's doing is fundamentally different. Numpy squares all elements of `xarray`, making a new, unnamed, temporary array `xarray**2`. Then it does the same with `yarray` to make `yarray**2`. Then it adds the two temporary arrays, and finally it computes the square root of each element (note that this is `numpy.sqrt` and not `math.sqrt`). Numpy has allocated a lot of temporary arrays, but in the end, it's 37 times faster than the pure Python version (on my machine with a million-element list).

If we think of our data schematically as a table with two columns (`x` and `y`) and a million rows, the pure Python implementation steps row-by-row and computes the full expression individually. The Numpy implementation computes each operation of the expression a whole column (or temporary column) at a time. The Numpy syntax is great because it makes the fast column-wise algorithm look just like the slow row-wise algorithm.

However, this syntax doesn't work if the calculation is anything but a table transformation (same number of output rows, each row is transformed independently). Numpy has a nice syntax for filtering rows (with square brackets, like R), and this brings us up to the complexity of SQL with `SELECT` and `WHERE` but no `GROUP BY`. Here are some things that are hard to translate unless there is a Numpy function that does exactly what you want:

   * Rows containing arbitrary length objects, such as events containing lists of jets, or events that have <i>N</i> tracks that have <i>N<sub>i</sub></i> hits.
...
Numpy has a built-in `numpy.histogram` for the simple case, but not for anything more complicated.
...
In the pure Python case, we have a big list of individual rows and the lambda takes one row (`datum`) each time it is invoked (which is many times). In the Numpy case, we have a single dict of column arrays and the lambda takes the whole collection when it is invoked (once).
...
In my example above, the Numpy implementation extracts all data from a dict of Numpy columns. It could have been an object with Numpy columns as object attributes (and the string-based function would work in the same way), or it could have been anything else. The job of producing one-dimensional Numpy arrays from `data`, whatever `data` happens to be, is up to the lambda functions. If you have a weird data source, you need correspondingly weird lambda functions.
...
Also, if the dataset is small or empty, Numpy may lose due to time spent setting up the calculation. As usual, optimization is driven by particular cases, which is why it's good that the same Histogrammar construct runs in each mode. You can test both without rewriting your analysis, which is not the case for general Numpy translations!

Here is a rough table of speedup factors (wall time for pure Python divided by wall time for Numpy) for primitives in typical applications. For instance, it's possible to use `Stack` on a suite of `Counts`, but it's usually used on a suite of histogrmas (`Bin` of `Count`).

| Primitive         | Numpy speedup factor |
|:------------------|:-----------------|
| Count             | ~100X            |
| Sum               | 40-100X          |
| Average           | 40-100X          |
...
| Maximize          | 50-150X          |
| Quantile          | 1-5X             |
| Bin               | 5-25X (for 100 bins) |
| SparselyBin       | 4-5X (about 100 bins) |
| CentrallyBin      | 25-40X (10 bins) |
| AdaptivelyBin     | _not possible!_  |
| Categorize        | 1.5X             |
| Fraction          | 4-20X (100 bins) |
| Stack             | 2-12X (10 plots) |
| Partition         | 1-4X (10 plots)  |
| Select            | 4-20X (100 bins) |
| Limit             | pass-through     |
| Label             | pass-through     |
| UntypedLabel      | pass-through     |
| Index             | pass-through     |
| Branch            | pass-through     |
| Bag               | 1.5-2X           |
| Sample            | 1-1.5X           |
